...
@Component({
  selector: 'app-tiketscount',
  templateUrl: './ticketscount.component.html',
  styleUrls: ['./ticketscount.component.css']
})
export class TicketscountComponent implements OnInit {
...
if(type=="res" && this.startDate1=="")
{
  document.getElementById("divv").innerHTML=" ";
  var lineData = [];
 	this._httpService.  getResolvedDetails().subscribe((res:any[])=>{
...
else if(type=="pen" && this.startDate1=="")
{
  document.getElementById("divv").innerHTML=" ";
  var lineData = [];
 	this._httpService.  getPendingDetails().subscribe((res:any[])=>{
    this.startDate=""+(new Date(res[0].date).getFullYear())+"-0"+(new Date(res[0].date).getMonth()+1)+"-0"+(new Date(res[0].date).getDate());
     this.endDate=""+(new Date(res[res.length-1].date).getFullYear())+"-0"+(new Date(res[res.length-1].date).getMonth()+1)+"-0"+(new Date(res[res.length-1].date).getDate());
     for(let i=res.length-1;i>=(res.length-7);i--){
...
else if(type=="assign" && this.startDate1=="")
{
  document.getElementById("divv").innerHTML=" ";
  var lineData = [];
 	this._httpService.  getAssignedDetails().subscribe((res:any[])=>{
    this.startDate=""+(new Date(res[0].date).getFullYear())+"-0"+(new Date(res[0].date).getMonth()+1)+"-0"+(new Date(res[0].date).getDate());
     this.endDate=""+(new Date(res[res.length-1].date).getFullYear())+"-0"+(new Date(res[res.length-1].date).getMonth()+1)+"-0"+(new Date(res[res.length-1].date).getDate());
     for(let i=res.length-1;i>=(res.length-7);i--){
       var last_date=""+(new Date(res[i].date).getFullYear())+"-0"+(new Date(res[i].date).getMonth()+1)+"-"+(new Date(res[i].date).getDate());
...
else if(type=="res" && this.startDate1!="")
{
  document.getElementById("divv").innerHTML=" ";
...
svg.append('text')                                     
  ...
  if(this.type1=='res' || this.type1==null)
  {
...
svg.append('text')                                     
  ...
  constructor(private router: Router,public authService: AuthService,  private _httpService:TicketscountService) { 
